...
@testset "gamma pdf, cdf, etc." begin
    kk = [1.5, 2.0, 5.0, 10.0]
    θθ = [1.0, 0.5, 0.1, 0.01]
...
@testset "gamma expert_ll" begin
    kk = [1.5, 2.0, 5.0, 10.0]
    θθ = [1.0, 0.5, 0.1, 0.01]
    x = collect(0.0:0.5:100.0)

    for k in kk, θ in θθ
        tmp = LRMoE.GammaExpert(k, θ)
...
@testset "gamma exposurize" begin
    d = GammaExpert()
...
@testset "gamma lev/excess" begin
    kk = [1.5, 2.0, 5.0, 10.0]
    θθ = [1.0, 0.5, 0.1, 0.01]
    x = [0.0, Inf]
    ...
    for k in kk, θ in θθ
        ...
        @test LRMoE.excess(tmp, 0.0) == mean(Gamma(k, θ))
        @test LRMoE.lev(tmp, Inf) == mean(Gamma(k, θ))

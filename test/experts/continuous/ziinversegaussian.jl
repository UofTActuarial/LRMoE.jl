@testset "ziinversegaussian pdf, cdf, etc." begin
    pp = [0.05, 0.10, 0.50, 0.90]
    μμ = [1.0, 2.0, 5.0, 10.0]
    λλ = [1.0, 0.5, 0.1, 0.01]
    x = collect(0.0:0.5:100.0)

    for p in pp, μ in μμ, λ in λλ
        l = Distributions.InverseGaussian(μ, λ)
        r = LRMoE.ZIInverseGaussianExpert(p, μ, λ)
...
@testset "ziinversegaussian expert_ll" begin
    pp = [0.05, 0.10, 0.50, 0.90]
    μμ = [1.0, 2.0, 5.0, 10.0]
    λλ = [1.0, 0.5, 0.1, 0.01]
    x = collect(0.0:0.5:100.0)
    ...
    for p in pp, μ in μμ, λ in λλ
        tmp = LRMoE.ZIInverseGaussianExpert(p, μ, λ)
...
@testset "ziinversegaussian exposurize" begin
    d = ZIInverseGaussianExpert()
...
@testset "ziinversegaussian lev/excess" begin
    pp = [0.05, 0.10, 0.50, 0.90]
    μμ = [1.0, 2.0, 5.0, 10.0]
    λλ = [1.0, 0.5, 0.1, 0.01]
    x = [0.0, Inf]
    ...
    for p in pp, μ in μμ, λ in λλ
        tmp = ZIInverseGaussianExpert(p, μ, λ)
        @test LRMoE.lev(tmp, 0.0) == 0.0
        @test LRMoE.excess(tmp, 0.0) == (1 - p) * mean(InverseGaussian(μ, λ))
        @test LRMoE.lev(tmp, Inf) == (1 - p) * mean(InverseGaussian(μ, λ))

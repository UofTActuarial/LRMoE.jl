@testset "zigamma pdf, cdf, etc." begin
...
@testset "zigamma expert_ll" begin
    pp = [0.05, 0.10, 0.50, 0.90]
    kk = [1.5, 2.0, 5.0, 10.0]
    θθ = [1.0, 0.5, 0.1, 0.01]
    x = collect(0.0:0.5:100.0)
...
@testset "zigamma exposurize" begin
    d = ZIGammaExpert()
...
@testset "zigamma lev/excess" begin
    pp = [0.05, 0.10, 0.50, 0.90]
    kk = [1.5, 2.0, 5.0, 10.0]
    θθ = [1.0, 0.5, 0.1, 0.01]
    x = [0.0, Inf]
    ...
    for p in pp, k in kk, θ in θθ
        tmp = ZIGammaExpert(p, k, θ)
        @test LRMoE.lev(tmp, 0.0) == 0.0
        @test LRMoE.excess(tmp, 0.0) == (1 - p) * mean(Gamma(k, θ))
        @test LRMoE.lev(tmp, Inf) == (1 - p) * mean(Gamma(k, θ))

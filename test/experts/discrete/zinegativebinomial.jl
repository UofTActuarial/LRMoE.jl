@testset "zinegativebinomial pdf, cdf, etc." begin
    ...
    for p0 in p0p0, n in nn, p in pp
        ...
        r = LRMoE.ZINegativeBinomialExpert(p0, n, p)
...
@testset "zinegativebinomial expert_ll" begin
    p0p0 = [0.01, 0.05, 0.10, 0.50, 0.90, 0.95, 0.99]
    nn = [1, 2, 5, 10, 25, 50, 100]
    pp = [0.01, 0.05, 0.10, 0.50, 0.90, 0.95, 0.99]
    x = collect(0.0:1:50.0)
    ...
    for p0 in p0p0, n in nn, p in pp
        tmp = LRMoE.ZINegativeBinomialExpert(p0, n, p)
...
@testset "zinegativebinomial exposurize" begin

@testset "binomial pdf, cdf, etc." begin
...
@testset "binomial expert_ll" begin
    nn = [1, 2, 5, 10, 25, 50, 100]
    pp = [0.01, 0.05, 0.10, 0.50, 0.90, 0.95, 0.99]
    x = collect(0.0:1:50.0)

    for n in nn, p in pp
        tmp = LRMoE.BinomialExpert(n, p)
...
@testset "binomial exposurize" begin
    d = BinomialExpert()
